...
def georef_ortho(ortho: np.ndarray,
    ...
    if debug_display_start:
        style_config = {
            "title": "Aligned footprints",
        }
        di.display_images([ortho],
                          footprints=aligned_footprints,
    ...
    if points.shape[0] < min_nr_tps:
        print(f"Not enough tie points ({points.shape[0]}) found")
        return None, None

    if debug_show_final_tie_points:

        style_config = {
            "title": f"{points.shape[0]} tie points found at rotation {best_rotation}",
        }
        di.display_images([ortho],
                          points=[points[:, 2:4]],
                          style_config=style_config)
