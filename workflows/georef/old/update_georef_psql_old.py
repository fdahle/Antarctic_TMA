...
def update_georef_psql():
    ...
    if overwrite is False:
        sql_string += " WHERE position_exact IS NULL"
    position_approx = ctd.execute_sql(sql_string, conn)
    ...
    for i, row in tqdm(footprints.iterrows(), total=footprints.shape[0]):
        ...
        row_approx = position_approx[position_approx["image_id"] == image_id]

        if row_azimuth.shape[0] == 0:
            print("AZIMUTH NOT FOUND")
            continue
