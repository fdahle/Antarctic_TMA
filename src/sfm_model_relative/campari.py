...
def campari(project_folder, m_args,
    ...
    # the actual calling of campari
    shell_string = f'mm3d Campari "{m_args["ImagePattern"]}" {m_args["InputOrientation"]} ' \
                   f'{m_args["OutputOrientation"]}'
    ...
    print("Finished with Campari")

    if save_stats:
        with open(stats_folder + "/campari.json", "w") as outfile:
