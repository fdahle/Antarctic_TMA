...
def convert_image_to_footprint(img, image_id, transform, catch=True, verbose=False, pbar=None):
    ...
    Args:
        img (np-array): The image we want to convert to a approx_footprint
...
if __name__ == "__main__":
    ...
    img_id = "CA214732V0030"
    _img_path = f"/data_1/ATM/temp/georeference/images/from_ortho/{img_id}.tif"

    import base.load_image_from_file as liff
    _img, _transform = liff.load_image_from_file(_img_path, return_transform=True)

    poly = convert_image_to_footprint(_img, img_id, _transform)
