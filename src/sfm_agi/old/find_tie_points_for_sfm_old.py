...
def find_tie_points_for_sfm(img_folder,
    ...
    image_ids = [img_id for img_id in image_ids if img_id in image_dims.keys()]
    ...
    for idx, (img_1_id, img_2_id) in (pbar := tqdm(enumerate(combinations), total=len(combinations))):

        pbar.set_description("Find tie-points")

        if not (img_1_id == "CA213732V0072" or img_2_id == "CA213731L0072"):
            continue
        ...
        if use_cached_tps and img_1_id + "_" + img_2_id in cached_tps:
            ...
            pbar.set_postfix_str(f"{tps.shape[0]} tps between {img_1_id} and {img_2_id} (cached)")
        ...
        # we need to calculate the tie points
            ...
            if rotation_dict is not None:
                ...
                # rotation is only required if both! images are vertical
            ...
            if debug_show_final_tps:
                ...
                if img_1_id == "CA213732V0072" and img_2_id == "CA213733R0072" or \
                     img_1_id == "CA213733R0072" and img_2_id == "CA213732V0072":
            ...
            pbar.set_postfix_str(f"{tps.shape[0]} tps between {img_1_id} and {img_2_id}")
        ...
        if np.any(tps < 0):
            raise ValueError(f"Negative tie points found in tps for {img_1_id} and {img_2_id}")
