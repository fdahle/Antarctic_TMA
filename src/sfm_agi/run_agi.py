...
def run_agi(project_name, images,
    ...
    if STEPS["create_masks"]:
        ...
        # detect fiducials
        chunk_temp.detectFiducials(generate_masks=True, generic_detector=False,
                                   frame_detector=True, fiducials_position_corners=False)
    ...
    if STEPS["create_bounding_box"]:
        ...
        bounding_box_relative = Metashape.BBox(min_corner, max_corner)  # noqa
    else:
        bounding_box_relative = None
    ...
    if STEPS["build_dem"]:
        ...
        if bounding_box_relative is not None:
            build_params['region'] = bounding_box_relative
        ...
        if bounding_box_relative is not None:
            export_params['region'] = bounding_box_relative
    ...
    if STEPS["create_gcps"]:
        ...
        eg.export_gcps(dem, ortho, bounding_box_relative, zoom_level_dem,
                       resolution_relative, footprints, gcp_path)
    ...
    if STEPS["build_pointcloud"]:
        ...
        # build dense cloud
        chunk.buildPointCloud(replace_asset=True)
        doc.save()

        output_pc_path = os.path.join(output_fld, project_name + "_pointcloud_absolute.laz")
    ...
    if STEPS["build_dem"]:
        ...
        if bounding_box_absolute is not None:
            build_params['region'] = bounding_box_absolute
        ...
        if bounding_box_absolute is not None:
            export_params['region'] = bounding_box_absolute

        chunk.exportRaster(**export_params)

        print("TODO CALC DIFFERENCE AND SAVE")
