...
def georef_img(image_id, path_fld, path_georef_tiffs,
    ...
    for i, next_footprint in enumerate(next_footprints_exact):
        ...
        if next_footprint is not None:
            ...
            next_image, next_transform = liff.load_image_from_file(next_path, driver="gdal",
                                                                   return_transform=True,
                                                                   catch=catch)
            if enhance_image:
                next_image = eh.enhance_image(next_image, scale=(enhancement_min, enhancement_max))
    ...
    image_orig = copy.deepcopy(image_no_borders)
    image_orig = ri.rotate_image(image_orig, data['azimuth'], catch=catch, verbose=False, pbar=pbar)
    if enhance_image:
        image_no_borders = eh.enhance_image(image_no_borders, scale=(enhancement_min, enhancement_max))
    ...
    if debug_show_tie_points_final:
        di.display_images([image], points=all_tps[:, 0:2], point_size=25,
                          title=f"{all_tps.shape[0]} final tie-points found")

    if status.startswith("too_few_tps") is False:

        # Swap columns
        all_tps[:, [0, 1, 2, 3]] = all_tps[:, [2, 3, 0, 1]]

        # get the transform
        transform, residuals = ag.apply_gcps(path_tiff, image_orig, all_tps,
        ...
        footprint = citf.convert_image_to_footprint(image_orig, image_id, transform,
